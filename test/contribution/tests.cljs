...
(defn state-call! [instance method from-address-index ether-value & [method-args]]
  (apply contract-call instance method (concat method-args [{:gas gas-limit
...
(defn- d0x-balance-of [address]
  (wei->eth (contract-call D0xToken :balance-of address)))
...
(deftest contracts-setup
  ...
  (is (= 250000000 (wei->eth (contract-call D0xToken :total-supply))))
  (is (not (contract-call D0xToken :transfers-enabled)))
  ...
  (let [soft-cap-amount (eth->wei 10)
    ...
    (testing "Should be able to set contribution period"
      ...
      (let [contrib-period (get-contrib-period 0)]
        (is (= (wei->eth soft-cap-amount) (:contrib-period/soft-cap-amount contrib-period)))
    ...
    (testing "Shouldn't be able to contribute before contribution start time"
      (is (thrown? js/Error (state-call! Contribution :contribute 10 1))))
    ...
    (async done
      (go
          ...
          (let [wallet-balance-before (account-eth-balance WALLET)
            ...
            (testing "Users should be able to contribute"
              (doseq [[acc-index amount] contributions]
                (state-call! Contribution :contribute acc-index amount))
              ...
              (doseq [[acc-index amount] contributions]
                (let [[contrib-amount compensated?] (contract-call Contribution :get-contributor 0 (account acc-index))]
                  (is (= (wei->eth contrib-amount) amount))
        ...
        (testing "Contribution round 2"
          (let [wallet-balance-before (account-eth-balance WALLET)
                soft-cap-amount (eth->wei 3)
                hard-cap-amount (eth->wei 6)
              ...
              (testing "Hitting soft cap should change contrib period end time"
                (let [contrib-period (get-contrib-period 1)]
                  (is (<= (+ start-time after-soft-cap-duration)
                          (to-epoch (:contrib-period/end-time contrib-period))
                ...
                (state-call! Contribution :set-contrib-period OWNER1 0
                             (u/contrib-period-args
                               {:contribution/period-index 2
                                :contrib-period/soft-cap-amount (eth->wei 3)
                                :contrib-period/after-soft-cap-duration (time/in-seconds (time/seconds 2))
                                :contrib-period/hard-cap-amount (eth->wei 5)
...
(deftest contribution2
  (let [soft-cap-amount (eth->wei 1)
        after-soft-cap-duration (time/in-seconds (time/hours 48))
        hard-cap-amount (eth->wei 2)
        start-time (now-plus-seconds 4)
        end-time (now-plus-seconds 8)
        wait-ch (chan)]
    (testing "Should be able to set contribution period"
          ...
          (testing "Contribution round 1"
            (let [wallet-balance-before (account-eth-balance WALLET)
              ...
              (testing "Users should be able to contribute and hit hard cap"
                (doseq [[acc-index amount] (take 2 contributions)]
                  (state-call! Contribution :contribute acc-index amount))
                (mine-block)
              ...
              (testing "After hitting hard cap users shouldn't be able to contribute"
                (doseq [[acc-index amount] (drop 2 contributions)]
                  (is (thrown? js/Error (state-call! Contribution :contribute acc-index amount)))))
              ...
              (testing "Should be able to enable d0x token transfers"
                (state-call! Contribution :enable-d0x-token-transfers OWNER1 0))

              (testing "Trading d0x should be enabled"
                (state-call! D0xToken :transfer 11 0 [(account 12) (eth->wei 1)])
                (is (= 1 (wei->eth (u/big-num->num (contract-call D0xToken :balance-of (account 12)))))))

              (testing "Vesting works"
                (is (thrown? js/Error (state-call! D0xToken :transfer FOUNDER1 0 [(account FOUNDER2) (eth->wei 1)]))))))
          (done))))))
